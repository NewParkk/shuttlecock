...
<body>
	<div class="container">
		...
		<c:choose>
			<c:when test="${not empty leagueboardList}">
				<c:forEach items="${leagueboardList}" var="leagueboard">
					<div class="post-preview"
						onclick="location.href='/LeagueBoard/${leagueboard.leagueboardId}'">
						<h3>${leagueboard.title}</h3>
						<p>${fn:substring(leagueboard.content, 0, 10)}...</p>
					</div>
				</c:forEach>
			</c:when>
			<c:otherwise>
